...
export const SchoolPublicHomeView: React.FC<Props> = ({ setActiveTab }) => {
  ...
  const getCardStyleClasses = () => {
    switch (cards.cardStyle) {
      case 'elevated':
        return 'shadow-md hover:shadow-xl border border-slate-100 bg-white';
      case 'flat':
        return 'border-0 bg-slate-100/90 hover:bg-slate-200/80 shadow-none';
      case 'gradient':
        return 'border border-blue-100/90 bg-gradient-to-br from-white to-blue-50/40 shadow-xs hover:shadow-md';
      case 'bordered':
      default:
        return 'border border-slate-200 shadow-xs hover:shadow-md hover:border-blue-300 bg-white';
    }
  };

  const getDensityPadding = () => {
    switch (cards.cardDensity) {
      case 'compact':
        return 'p-3.5';
      case 'spacious':
        return 'p-7';
  ...
  const isAdmin = currentRole === 'admin_sekolah' || currentRole === 'super_admin';

  return (
    <div className="space-y-6 sm:space-y-8 animate-in fade-in duration-300 pb-12">
      {/* Admin Quick Customizer Notice Banner */}
      {isAdmin && (
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3 sm:px-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
      ...
      {/* Quick Action Interactive Cards (Toggled by cards.showQuickActions) */}
      {cards.showQuickActions && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {cards.showAcademicSummary && (
              <div
                onClick={() => setActiveTab('akademik')}
                className={`group rounded-2xl transition cursor-pointer flex flex-col justify-between ${getCardStyleClasses()} ${getDensityPadding()}`}
              >
                <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Left 2 Cols: School Announcements (Toggled by cards.showAnnouncementsCard) */}
        {cards.showAnnouncementsCard && (
          <div className={`lg:col-span-2 rounded-2xl ${getCardStyleClasses()} ${getDensityPadding()}`}>
            <div className="flex items-center justify-between mb-4">
        ...
        <div className={`rounded-2xl flex flex-col justify-between space-y-4 ${cards.showAnnouncementsCard ? 'lg:col-span-1' : 'lg:col-span-3'} ${getCardStyleClasses()} ${getDensityPadding()}`}>
          <div>
